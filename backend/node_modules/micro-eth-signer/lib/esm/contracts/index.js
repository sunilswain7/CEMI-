...
for (let item of ERC20) {
    ...
    COMMON_TOKENS[addr] = { abi: 'ERC20', symbol, decimals: decimals || 18, price };
}
export function tokenFromSymbol(symbol) {
    for (let c in COMMON_TOKENS) {
        if (COMMON_TOKENS[c].symbol === symbol)
            return Object.assign({ contract: c }, COMMON_TOKENS[c]);
    }

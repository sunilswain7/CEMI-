...
function basePaths(a, b, exactOutput = false) {
    ...
    for (let c of BASES) {
        for (let fee1 in Fee) {
            for (let fee2 in Fee) {
                let path = [wA, packFee(fee1), c.contract, packFee(fee2), wB].map((i) => hex.decode(abi.strip0x(i)));
                if (exactOutput)
                    path = path.reverse();
...
export async function bestPath(net, a, b, amountIn, amountOut) {
    ...
    for (let i of paths) {
        ...
        i[amountIn ? 'amountOut' : 'amountIn'] = quoter['quoteExact' + (amountIn ? 'Input' : 'Output') + (i.path ? '' : 'Single')].call(opt);
    }
...
const ROUTER_CONTRACT = abi.contract(contracts.UNISWAP_V3_ROUTER, undefined, contracts.UNISWAP_V3_ROUTER_CONTRACT);

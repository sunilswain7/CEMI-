...
export function mapComponent(c) {
    ...
    if ((m = ARRAY_RE.exec(c.type))) {
        ...
        if (m[3] !== undefined) {
            ...
            // Dynamic array
            return P.pointer(PTR, wrappedArray(P.U256BE, inner));
        }
    }
    if (c.type === 'tuple') {
    ...
    if (c.type === 'string')
        return P.pointer(PTR, P.padRight(32, P.string(P.U256BE), P.ZeroPad));
    if (c.type === 'bytes')
        return P.pointer(PTR, P.padRight(32, P.bytes(P.U256BE), P.ZeroPad));
    if (c.type === 'address')
        return EPad(P.hex(20, false, true));
    if (c.type === 'bool')
        return EPad(P.bool);
...
export function contract(abi, net, contract) {
    ...
    for (let fn of abi) {
        ...
        const encodeInput = (v) => P.concatBytes(hex.decode(sh), inputs ? inputs.encode(v) : new Uint8Array());
        ...
        res[name].call = async (args, overrides = {}) => {
            ...
            return decodeOutput(hex.decode(strip0x(await net.ethCall(Object.assign({ to: contract, data: add0x(hex.encode(input)) }, overrides)))));
        };
        res[name].estimateGas = async (args, overrides = {}) => {
            const input = encodeInput(args);
            if (!contract && !overrides.to)
                throw new Error('No contract address');
            return await net.estimateGas(Object.assign({ to: contract, data: add0x(hex.encode(input)) }, overrides));
        };
    }
    return res;
}
    ...
    for (let elm of abi) {
        ...
        res[elm.name] = {
            decode(topics, _data) {
                ...
                const indexedParsed = indexed.map((p, i) => p ? p.decode(hex.decode(strip0x(topics[i]))) : topics[i]);
            ...
            topics(values) {
                ...
                for (let i = 0, ii = 0; i < inputs.length && ii < indexed.length; i++) {
                    ...
                    let topic;
                    if (packer)
                        topic = hex.encode(packer.encode(value));
                    else if (['string', 'bytes'].includes(input.type))
                        topic = hex.encode(keccak_256(value));
                    else {
                        let m, parts;

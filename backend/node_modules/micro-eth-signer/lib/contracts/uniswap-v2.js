...
const hints = {
    swapETHForExactTokens(v, opt) {
        ...
        return `Swap up to ${abi.Decimal.encode(opt.amount)} ETH for exact ${last}. ${uniTs(v.deadline)}`;
    },
    ...
    swapExactTokensForETH(v, opt) {
        ...
        if (!first)
            throw new Error('Not enough info');
        return `Swap exact ${first} for at least ${abi.Decimal.encode(v.amountOutMin)} ETH. ${uniTs(v.deadline)}`;
    },
    ...
    swapTokensForExactETH(v, opt) {
        ...
        if (!first)
            throw new Error('Not enough info');
        return `Swap up to ${first} for exact ${abi.Decimal.encode(v.amountOut)} ETH. ${uniTs(v.deadline)}`;
    },
...
(0, common_js_1.addHints)(ABI, hints);
exports.default = ABI;

...
exports.parse = parse;
exports.parseFromBytes = parseFromBytes;
/**
...
const findDescriptors_js_1 = require("./findDescriptors.js");
const VirtualSize_js_1 = require("../VirtualSize.js");
/**
 * Try to get an address from a script, returning null for non-standard outputs.
 */
function tryGetAddress(script, coin) {
    try {
        return (0, address_js_1.fromOutputScriptWithCoin)(script, coin);
    }
    catch {
        return null;
    }
 ...
 */
function parse(psbt, descriptorMap, coin) {
    const rawInputs = psbt.getInputs();
    const rawOutputs = psbt.getOutputs();
    let totalInputValue = 0n;
    let totalOutputValue = 0n;
    let spendAmount = 0n;
    const inputs = rawInputs.map((inputData, i) => {
        const witnessUtxo = inputData.witnessUtxo;
        if (!witnessUtxo) {
            throw new Error(`Missing witnessUtxo for input ${i}`);
        }
        const scriptIdResult = (0, findDescriptors_js_1.findDescriptorForInput)(inputData, descriptorMap);
        if (!scriptIdResult) {
        ...
        return {
            address: (0, address_js_1.fromOutputScriptWithCoin)(witnessUtxo.script, coin),
            value: witnessUtxo.value,
            scriptId,
        };
    });
    const outputs = rawOutputs.map((outputData) => {
        const scriptIdResult = (0, findDescriptors_js_1.findDescriptorForOutput)(outputData.script, outputData, descriptorMap);
        const scriptId = scriptIdResult
            ? {
                descriptor: scriptIdResult.descriptor,
                index: scriptIdResult.index,
            }
    ...
    const virtualSize = (0, VirtualSize_js_1.getVirtualSize)({
 ...
 * @returns Parsed transaction information
 */
function parseFromBytes(psbtBytes, descriptorMap, coin) {
    const psbt = index_js_1.Psbt.deserialize(psbtBytes);
